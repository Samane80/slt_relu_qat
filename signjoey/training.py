...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict) -> None:
        ...
        # resume=True (i.e. keep an existing model_dir instead of raising or
        # wiping it) whenever training.load_model is set -- that's exactly
        # the "continue an interrupted/previous run" case.
        self.model_dir = make_model_dir(
            train_config["model_dir"],
            overwrite=train_config.get("overwrite", False),
            resume="load_model" in train_config.keys(),
        )
        ...
        # ReLUFormer attention regularization loss weight (Eq. 7 of the
        # ReLUFormer paper). No effect unless encoder/decoder use
        # "reluformer" attention_type; ignored (0.0) otherwise.
        self.attn_reg_loss_weight = train_config.get("attn_reg_loss_weight", 0.0)
    ...
    def _save_checkpoint(self) -> None:
        ...
        symlink_update(
        os.path.join(self.model_dir, "{}.ckpt".format(self.steps)),
        os.path.join(self.model_dir, "best.ckpt"),)
    ...
    def _train_batch(self, batch: Batch, update: bool = True) -> (Tensor, Tensor):
        ...
        recognition_loss, translation_loss = self.model.get_loss_for_batch(
            ...
            attn_reg_loss_weight=self.attn_reg_loss_weight,
        )
